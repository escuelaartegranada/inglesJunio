...
export default function DragDrop({ onNavigate }: Props) {
  ...
  return (
    ...
    >
      <div className="flex-1 w-full flex flex-col items-center justify-center p-4 max-w-5xl gap-10">
        
        {/* The Picture and Drop Zone */}
        <div className="bg-white rounded-[48px] p-8 shadow-xl border-4 border-amber-200 flex flex-col items-center gap-8 w-full max-w-md">
          <div className="bg-amber-50 w-full py-12 rounded-[32px] flex items-center justify-center border-2 border-amber-100">
             <div className="w-48 h-48 bg-amber-200 rounded-full flex items-center justify-center shadow-inner">
               {IconComponent && <IconComponent size={96} className="text-amber-700" strokeWidth={2} />}
             </div>
          </div>
          
          <div 
            ref={dropZoneRef}
            className={`w-[90%] h-28 rounded-[32px] border-4 border-dashed transition-colors flex items-center justify-center
              ${isPlaced ? 'border-emerald-500 bg-emerald-50' : 'border-amber-300 bg-orange-50'}`}
          >
             {isPlaced && (
               <span className="text-[40px] font-black text-emerald-700 uppercase tracking-widest">{currentItem.en}</span>
             )}
          </div>
        </div>
        ...
        {!isPlaced && (
          <div className="flex flex-col items-center gap-8 w-full mt-8">
            <h3 className="text-3xl font-bold text-amber-600 mb-4 bg-amber-100 px-8 py-3 rounded-full border-4 border-amber-200">
